...
#p_thresh <- as.numeric(args[8]) always 1
n_sample <- as.numeric(args[8])
rev  <- as.logical(args[9])
nb_files = args[-c(1:9)]
...
if(norm_type == "ss"){
    if(type == "plain"){
        f <- fit_ff_new(B_std = B_hat, N = n, kmax = kmax, reverse_fit_order = rev)
    }else if(type == "ff"){
        f <- fit_ff_new(B_std = B_hat, N = n, R = R$R, kmax = kmax, reverse_fit_order = rev)
    }else if(type == "ev"){
        f <- fit_ev(B_hat, S_hat, n, R$R, kmax=kmax, adjust=FALSE)
    }
}else if(norm_type == "z"){
    if(type == "plain"){
        f <- fit_ff_new(Z_hat = B_hat, kmax = kmax, reverse_fit_order = rev)
    }else if(type == "ff"){
        f <- fit_ff_new(Z_hat = B_hat,R = R$R, kmax = kmax, reverse_fit_order = rev)
    }else if(type == "ev"){
